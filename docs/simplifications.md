...
- **Simplified:** The ⊙/⊗ marks follow each limb's current in time with the high side's power-factor angle, the high and low sides exactly opposite; the small magnetizing current that makes the difference, and any current circulating in the delta tertiary, are not shown. **Full treatment:** Each winding's own current from a model with the core's shunt branch and the delta's circulating (zero-sequence) current.
