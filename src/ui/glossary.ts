...
export const TERMS: Term[] = [
  ...
  T('core', 'Core', 'The steel inside a transformer that carries its magnetic flux from one winding to the other; built of thin insulated sheets so the flux cannot drive currents round inside the steel.', '', 'Steel carries flux thousands of times more easily than air or oil, so almost all of it stays in the core.'),
  T('winding', 'Winding', 'A coil of insulated copper wound around a transformer’s core; one per voltage, per phase.'),
  T('turn', 'Turn', 'One loop of a winding around the core. Every turn on a limb carries the same voltage, so a winding’s voltage is its turns times the volts per turn.', 'N'),
  T('flux', 'Magnetic flux', 'The magnetic field carried round a transformer’s core. The voltage on a winding sets it: it alternates with the voltage, and its size follows the voltage, not the load.', 'Φ (webers, Wb)', 'Faraday’s law: the voltage of a winding is its turns times how fast the flux changes, V = N dΦ/dt; at 60 Hz, E_rms = 4.44 f N Φ_max.'),
  T('turns-ratio', 'Turns ratio', 'The number of turns on one winding over the other’s. Voltages are in that ratio and currents in its inverse, so the power is the same on both sides less the losses.', 'a = N_H/N_L = V_H/V_L = I_L/I_H'),
  T('conservator', 'Conservator', 'A tank above a transformer’s lid, partly full of oil: as the oil warms and expands it rises into the conservator, so the main tank always stays full.'),
  T('radiator', 'Radiator', 'Thin steel panels on a transformer’s sides: warm oil flows through them and gives its heat to the air, then sinks back into the tank.'),
  T('tertiary', 'Tertiary winding', 'A third winding, connected in delta: it gives the transformer a path for the currents that keep the three phases’ voltages balanced when the loads are not.'),
  T('eddy-current', 'Eddy current', 'Current the alternating flux drives in circles inside solid metal, heating it; stacking the core from thin insulated sheets breaks those circles up.'),
  T('sf6', 'Sulfur hexafluoride', 'The gas inside most high-voltage breakers: it insulates several times better than air, and after an arc it becomes an insulator again very quickly.', 'SF₆'),
  T('arc', 'Arc', 'Current carried across a gap through hot, ionized gas. When a breaker’s contacts part, the current does not stop at once: it goes on across the gap as an arc until the breaker puts it out.'),
  T('current-zero', 'Current zero', 'The instant an alternating current passes through zero, twice every cycle (every 1/120 s at 60 Hz). An AC breaker interrupts there: the arc goes out, and the gas must hold off the voltage that returns.'),
  T('interrupting-time', 'Interrupting time', 'From the trip command to the current being out in every phase. High-voltage breakers are rated in cycles of the power frequency.'),
  T('ct', 'Current transformer', 'A ring round a conductor that gives a small current in proportion to the conductor’s: what protective relays and meters measure.', 'CT'),
  T('sag', 'Sag', 'How far a conductor hangs below the straight line between its two supports, at the middle of the span. It grows as the wire warms and lengthens.', 'D'),
  T('clearance', 'Clearance', 'The distance from a live conductor to the ground or to anything beneath it; codes set a minimum for each voltage, and the sag at the line’s hottest must respect it.'),
  T('ampacity', 'Ampacity', 'The current a conductor can carry while staying at or below its highest allowed temperature, in given weather: more in a cold wind, less in still, hot sun.', '', 'IEEE 738 computes it from the balance of the current’s heat and the sun’s against the heat the air carries away and the wire radiates.'),
  T('capacitor', 'Capacitor', 'Two conductors (plates) close together with insulation between them. A voltage across it puts equal and opposite charge on the plates and an electric field in the insulation; alternating voltage makes the charge, and so a current, flow in and out.', 'C'),
  T('dielectric', 'Dielectric', 'The insulation between a capacitor’s plates, which holds the electric field; in a power capacitor, thin polypropylene film soaked in insulating fluid.'),
  T('electric-field', 'Electric field', 'What charge sets up in the space around it: a push on other charges, from + toward −. Between a capacitor’s plates it is strong and uniform, and it is where the capacitor’s energy is stored.', 'E'),
  T('discharge-resistor', 'Discharge resistor', 'A resistor built in across a capacitor’s terminals that drains its charge after it is switched off, so it is safe to touch and to switch back in.'),
  T('ieee18', 'IEEE 18', 'The standard for shunt power capacitors: their ratings, tolerances, tests, and the discharge device each must have.'),
  T('shunt-reactor', 'Shunt reactor', 'A coil connected from a bus to ground that absorbs reactive power: the opposite of a capacitor bank, used where long lines’ own capacitance would push the voltage too high at light load.'),
  T('direct-current', 'Direct current', 'Current that flows one way only, as from a battery or a solar panel; the grid’s is alternating current, which reverses direction many times a second.', 'DC'),
  T('inverter', 'Inverter', 'A power-electronic converter that turns direct current (from solar panels or a battery) into alternating current in step with the grid, by switching it one way and the other many times a cycle.'),
  T('pwm', 'Pulse-width modulation', 'Switching a fixed voltage on and off, or one way and the other, in pulses whose widths follow the waveform wanted; averaged by a filter, the pulses become that waveform.', 'PWM'),
  T('h-bridge', 'H-bridge', 'Four switches arranged like the letter H between two DC rails, the load across the middle: closing opposite corners puts the DC across the load one way round, the other pair the other way.'),
  T('dc-link', 'DC link', 'The capacitors across an inverter’s DC side: they hold its voltage steady and buffer the difference between the steady power in and the pulsing power out.'),
  T('mppt', 'Maximum power point tracking', 'An inverter’s search for the voltage at which its panels give the most power in the sun and heat of the moment.', 'MPPT'),
  T('anti-islanding', 'Anti-islanding', 'An inverter’s duty to stop feeding the line within seconds when the grid goes away, so that it never keeps a de-energised line alive while crews work on it.'),
  T('ieee738', 'IEEE 738', 'The standard method for a bare overhead conductor’s temperature from its current and the weather, and so for the current it may carry.'),
