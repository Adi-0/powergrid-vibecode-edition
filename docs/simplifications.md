...
**What the model does.** Each dot is a fixed quantity of demand, scattered at random within a radius of the substation that carries it. The COUNT is real — it follows the model’s own load at the hour on show, and counting the dots gives the megawatts back. The positions are not: they are drawn once, out to a radius set by the site’s annual peak, so that the scatter fills in and empties as the day runs instead of re-rolling.
